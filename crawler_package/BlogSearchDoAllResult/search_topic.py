...
class SearchTopic(WeiboPage):
    ...
    def search_topic(self, tbwtuple):
        ...
        total_originator = [人民日报,环球时报,] 搜索的结果的首发者
        lfc_all_num = [该话题的点赞规模,该话题的转发规模,该话题的评论规模]
        """

        topic_words = tbwtuple  # 微博关键字
            ...
            for page in xrange(0, page_num):
                ...
                for item in issuer:
                    ...
                    if 'alt="V"/>' in item:
                        ...
                        total_originator.append(user_name)  # 搜索出来后的全部结果首发者

                        result_list = [blog_id, ptime, topic, content, user_id, user_name,
        ...
        return total_result_list, total_reason_list, total_comment_list, total_participants, total_originator, lfc_all_num
